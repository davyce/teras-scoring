...
import {
  ...
  X, Eye, Sparkles, BarChart3, Globe, TrendingUp, Shield,
  Search, Calendar,
} from 'lucide-react';
import { authFetch } from '../../utils/authFetch';
...
interface ParseResult {
  ...
  teras_signals: {
    ...
    income_signal: { monthly_avg_xaf: number };
  };
...
export default function GovernmentDocuments() {
  ...
  const handleUpload = async (files: FileList | null) => {
    if (!files?.length) return;
    const file = files[0];
    if (file.size > 30 * 1024 * 1024) { setError('Fichier trop volumineux (max 30 MB).'); return; }
    setUploading(true); setError('');
    const fd = new FormData();
    fd.append('file', file);
  ...
  const loadDetail = async (docId: string) => {
    if (selectedDoc === docId) { setSelectedDoc(null); setParseResult(null); setAnalysis(''); return; }
    setSelectedDoc(docId); setLoadingResult(true); setAnalysis(''); setShowTxns(false);
    try {
  ...
  const analyzeWithAI = async (docId: string) => {
    setAnalyzingDoc(docId); setAnalysis(''); setError('');
  ...
  const handleDelete = async (docId: string) => {
    if (!confirm('Supprimer ce document gouvernemental ?')) return;
  ...
  const renderAnalysis = (text: string) =>
    text.split('\n').map((line, i) => {
      if (line.startsWith('## ')) return <h3 key={i} className="text-sm font-bold text-sky-300 mt-4 mb-1.5">{line.slice(3)}</h3>;
      if (line.startsWith('**') && line.endsWith('**')) return <p key={i} className="text-sm font-bold text-white mt-2">{line.replace(/\*\*/g,'')}</p>;
      if (line.match(/^[-•]\s/)) return <div key={i} className="flex gap-2 ml-2 mt-0.5"><span className="text-sky-400 text-xs mt-0.5">◆</span><span className="text-sm text-slate-200">{line.slice(2)}</span></div>;
  ...
  return (
    <div className="min-h-screen bg-[#0b1220] text-white p-6">
      ...
      {/* Header */}
      <div className="mb-8">
        <p className="text-xs font-bold text-violet-400 uppercase tracking-widest mb-2">TERAS Gouvernement</p>
        <h1 className="text-3xl font-black text-white">Documents Économiques</h1>
        <p className="text-slate-400 mt-1 text-sm">
          Importez vos données nationales — L'IA TERAS les analyse et enrichit le tableau de bord CEMAC
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {DOC_TYPES.slice(0, 4).map(t => (
          <button key={t.value} onClick={() => setDocType(t.value)}
            ...
            {t.desc && <p className="text-slate-500 text-xs mt-0.5">{t.desc}</p>}
          </button>
        ))}
      </div>
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          ...
          <div>
            <label className="text-slate-400 text-xs mb-1.5 block">Période (ex: 2025-Q4)</label>
            <input value={period} onChange={e => setPeriod(e.target.value)}
              placeholder="Ex: 2025-Q4 ou 2025"
              className="w-full bg-slate-800 border border-slate-700 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-violet-500 placeholder-slate-600"/>
          </div>
          <div>
            <label className="text-slate-400 text-xs mb-1.5 block">Description (optionnelle)</label>
            <input value={description} onChange={e => setDescription(e.target.value)}
              placeholder="Ex: Recettes DGI décembre 2025"
              className="w-full bg-slate-800 border border-slate-700 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-violet-500 placeholder-slate-600"/>
          </div>
        </div>
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${
            dragOver ? 'border-violet-400 bg-violet-900/10' : 'border-slate-700 hover:border-slate-600 hover:bg-slate-800/30'
          }`}
        >
          <input ref={fileRef} type="file" accept=".pdf,.xlsx,.xls,.csv,.txt,.doc,.docx" className="hidden"
            onChange={e => handleUpload(e.target.files)}/>
          {uploading ? (
            <div className="flex items-center justify-center gap-2 text-violet-400">
              <Loader2 className="w-5 h-5 animate-spin"/> Upload et traitement en cours...
            </div>
          ) : (
            <div className="space-y-2">
              <Globe className="w-8 h-8 text-violet-400 mx-auto"/>
              <p className="text-white text-sm font-medium">Glissez vos données économiques ici</p>
              <p className="text-slate-500 text-xs">PDF · Excel · CSV · TXT · DOC/DOCX — Max 30 MB</p>
              <div className="flex items-center justify-center gap-1.5 text-xs text-slate-500 mt-1">
                <Sparkles className="w-3.5 h-3.5 text-violet-400"/>
                Analyse IA contexte gouvernement CEMAC — Claude Sonnet 4
              </div>
            </div>
          )}
        </div>
      ...
      <div className="flex gap-3 mb-5">
        <div className="relative flex-1">
          ...
          <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-slate-900/60 border border-slate-800 text-white rounded-xl text-sm focus:outline-none focus:border-violet-500 placeholder-slate-600"/>
        </div>
        <select value={filterType} onChange={e => setFilterType(e.target.value)}
          className="px-3 py-2.5 bg-slate-900/60 border border-slate-800 text-white rounded-xl text-sm focus:outline-none focus:border-violet-500">
          <option value="all">Tous types</option>
          {DOC_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
        </select>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12">
          <Loader2 className="w-5 h-5 text-violet-400 animate-spin mr-2"/><span className="text-slate-400">Chargement...</span>
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-12 bg-slate-900/40 border border-slate-800 rounded-2xl">
          ...
          <p className="text-slate-500 font-medium">Aucun document économique</p>
          <p className="text-slate-600 text-sm mt-1">Importez des données CSV ou Excel pour enrichir le tableau de bord CEMAC</p>
        </div>
      ) : (
          ...
          {filtered.map(doc => {
            ...
            return (
              <div key={doc.id} className={`border rounded-2xl overflow-hidden bg-slate-900/60 transition-all ${
                isOpen ? 'border-violet-500/40' : 'border-slate-800 hover:border-slate-700'
              }`}>
                <div className="flex items-center gap-3 px-4 py-3.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-white text-sm font-medium truncate">{doc.filename}</p>
                      <span className="px-2 py-0.5 bg-violet-900/40 border border-violet-700/40 text-violet-400 text-xs rounded-lg">
                        {DOC_TYPES.find(t => t.value === doc.doc_type)?.label || doc.doc_type}
                      </span>
                    </div>
                    <div className="flex items-center gap-3 mt-0.5 text-xs text-slate-500 flex-wrap">
                      <span className={doc.status === 'parsed' ? 'text-emerald-400' : doc.status === 'processing' ? 'text-amber-400' : 'text-slate-400'}>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    {doc.status === 'parsed' && (
                      <>
                        <button onClick={() => analyzeWithAI(doc.id)} disabled={!!analyzingDoc}
                          className="flex items-center gap-1.5 px-2.5 py-1.5 bg-violet-600 hover:bg-violet-500 disabled:opacity-50 text-white rounded-lg text-xs font-medium transition">
                          {analyzingDoc === doc.id ? <Loader2 className="w-3.5 h-3.5 animate-spin"/> : <Brain className="w-3.5 h-3.5"/>}
                          {analyzingDoc === doc.id ? '' : 'Analyser'}
                        </button>
                      </>
                    )}
                    <button
                      ...
                      <>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                          {[
                            ...
                            <div key={i} className="bg-slate-800/60 rounded-xl p-3">
                              ...
                              <p className={`text-${k.color}-400 font-bold text-sm`}>{k.value}</p>
                            </div>
                          ))}
                        </div>
                        ...
                        {parseResult.recommendations?.length > 0 && (
                          <div className="space-y-1.5">
                            {parseResult.recommendations.map((r, i) => (
                              <div key={i} className={`flex gap-2 px-3 py-2 rounded-xl text-xs ${
                                r.type === 'success' ? 'bg-emerald-900/20 border border-emerald-700/30 text-emerald-300'
                                : 'bg-slate-800/60 border border-slate-700/50 text-slate-400'
                              }`}>
                                <span>{r.type === 'success' ? '✅' : 'ℹ️'}</span>{r.message}
                              </div>
                            ))}
                          </div>
                        )}
                        ...
                        {parseResult.transactions && parseResult.transactions.length > 0 && (
                          <div>
                            <button onClick={() => setShowTxns(!showTxns)} className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-white">
                              ...
                              {showTxns ? 'Masquer' : 'Voir'} les {parseResult.transactions.length} lignes de données
                            </button>
                            {showTxns && (
                        ...
                        <button onClick={() => analyzeWithAI(doc.id)} disabled={!!analyzingDoc}
                          className="flex items-center gap-2 px-4 py-2 bg-violet-600 hover:bg-violet-500 disabled:opacity-50 text-white rounded-xl text-sm font-medium transition">
                          {analyzingDoc === doc.id
                            ? <><Loader2 className="w-4 h-4 animate-spin"/> Analyse en cours...</>
                            : <><Sparkles className="w-4 h-4"/> Analyse IA gouvernement</>}
                        </button>
                      </>
                    )}

                    {analyzingDoc === doc.id && !analysis && (
                      <div className="flex items-center gap-2 text-violet-400 text-sm">
                        <Loader2 className="w-4 h-4 animate-spin"/>
                        Claude Sonnet 4 analyse vos données économiques...
                      </div>
                    )}

                    {analysis && selectedDoc === doc.id && (
                      <div className="bg-slate-900/60 border border-violet-700/30 rounded-xl p-4">
                        <div className="flex items-center gap-2 mb-4 pb-3 border-b border-slate-800">
                          <div className="w-8 h-8 rounded-xl bg-violet-500/20 border border-violet-500/30 flex items-center justify-center">
                            <Brain className="w-4 h-4 text-violet-400"/>
                          </div>
                          <div>
                            <p className="text-white text-sm font-bold">Analyse IA Gouvernement — Claude Sonnet 4</p>
                            <p className="text-violet-400/70 text-xs">Analyse de niveau ministériel · CEMAC</p>
                          </div>
                          <div className="ml-auto flex items-center gap-1.5 px-2 py-1 bg-violet-900/30 border border-violet-700/40 rounded-full">
                            <div className="w-1.5 h-1.5 bg-violet-400 rounded-full animate-pulse"/>
                            <span className="text-violet-400 text-xs">Confidentiel</span>
                          </div>
                        </div>
      ...
      <DocumentPreviewModal
        isOpen={!!previewDoc}
        title={previewDoc ? `Document gouvernemental — ${previewDoc.filename}` : ''}
        fileName={previewDoc?.filename || ''}
        sourceUrl={previewDoc ? `/api/scoring/government/documents/${previewDoc.id}/download/` : ''}
        mode="auth-fetch"
