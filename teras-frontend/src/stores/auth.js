...
export const useAuth = create()(persist((set, get) => ({
    ...
    hasPermission: (permission) => {
        ...
        if (accountType === 'government') {
            const govUser = user;
            return govUser.permissions?.includes(permission) || false;
        }
        // Partner permissions (scopes)
        if (accountType === 'partner') {
            const partnerUser = user;
            return partnerUser.allowed_scopes?.includes(permission) || false;
        }
 ...
 */
export function useRouteAccess(allowedTypes) {
